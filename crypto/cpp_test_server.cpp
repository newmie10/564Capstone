...
// ── Key recovery (from your existing code) ───────────────────────────────────
...
static void split_fernet_key(const std::string& key_b64,
                              std::vector<uint8_t>& signing_key,
                              std::vector<uint8_t>& enc_key) {
    auto raw = base64url_decode(key_b64);
    if (raw.size() != 32)
        throw std::runtime_error("Fernet key must decode to exactly 32 bytes");
...
std::string fernet_encrypt(const std::string& plaintext, const std::string& key_b64) {
    ...
    split_fernet_key(key_b64, signing_key, enc_key);
...
std::string fernet_decrypt(const std::string& token_b64, const std::string& key_b64) {
    ...
    if (token.size() < 73)  throw std::runtime_error("Fernet token too short");
    if (token[0] != 0x80)   throw std::runtime_error("Unsupported Fernet version");
    ...
    if (CRYPTO_memcmp(expected_mac, token.data() + mac_offset, 32) != 0)
        throw std::runtime_error("HMAC verification failed — token tampered or wrong key");

    std::vector<uint8_t> iv(token.begin() + 9,  token.begin() + 25);
    std::vector<uint8_t> ct(token.begin() + 25, token.begin() + mac_offset);
    if (ct.size() % 16 != 0) throw std::runtime_error("Ciphertext not block-aligned");
    ...
    if (!EVP_DecryptFinal_ex(ctx, plaintext.data() + len1, &len2)) {
        EVP_CIPHER_CTX_free(ctx); throw std::runtime_error("EVP_DecryptFinal failed — bad padding");
    }
    EVP_CIPHER_CTX_free(ctx);
...
// Read a full HTTP request from a connected socket; return the raw body.
...
// POST `body` to http://host:port/path, block until 200 received.
static void http_post(const char* host, int port,
    ...
    if (connect(sock, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        close(sock);
        throw std::runtime_error(std::string("connect() failed for ") + host + ":" + std::to_string(port));
    }
...
int main() {
    ...
    // ── Send our own message back to Python ──────────────────────────────────
